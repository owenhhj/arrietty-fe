...
export default function TabContent({tabType}) {
  ...
  const addNewTagForm = () => {
    return (
      <TagListing tagInfo={getTemplate()} tagType={tabType} callback={handleChildTagOperation} createNewTag={true}
  ...
  const getTagListings = () => {
    ...
    if (tabType === "course") {
      allCourseTags.forEach(
        (tag) => {
          ret.push(
            <TagListing key={`courseId:${tag["id"]}`} tagData={tag} tagType={"course"}
                        callback={handleChildTagOperation} createNewTag={false}/>
          );
        }
      );
      ...
      allTextbookTags.forEach(
        (tag) => {
          ret.push(
            <TagListing key={`textbookId:${tag["id"]}`} tagData={tag} tagType={"textbook"}
                        callback={handleChildTagOperation} createNewTag={false} allCourseTags={allCourseTags}/>
          );
        }
      );
    } else {
      allOtherTags.forEach(
        (tag) => {
          ret.push(
            <TagListing key={`otherTagId:${tag["id"]}`} tagData={tag} tagType={"otherTag"}
                        callback={handleChildTagOperation} createNewTag={false}/>
          );
        }
      );
  ...
  return (
    <div className="tab-content">
      {addNew && (
        <MUIButton size={'small'} variant={1} label={`new ${getButtonName()}`} onClick={handleClickAddNew}/>
      )}
